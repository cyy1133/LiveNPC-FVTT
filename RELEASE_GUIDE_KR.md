...
배포 시 아래 5가지를 동시에 만족시키는 것이 목표입니다.

1. GitHub 방문자가 프로그램 목적을 바로 이해할 것
2. EXE 사용자 기준 실행 방법이 바로 보일 것
3. 최신 변경 사항이 무엇인지 확인할 수 있을 것
4. 소셜 연기와 전투 기능이 서로 모순 없이 설명될 것
5. 릴리스 파일과 문서가 서로 모순되지 않을 것
...
- README에 `소셜 연기`, `Scene Preset`, `다중 FVTT 세션`이 반영되어 있는가
- QUICKSTART가 README보다 짧고 실행 중심으로 정리되어 있는가
- `@NPC 이름:` 문법 예시가 문서에 포함되어 있는가
...
- `npm run dist` 통과
- EXE 설치 후 실행 가능
- `Start -> 소셜 반응 -> 전투 자동 턴 -> 턴 종료`가 정상 동작
- Scene preset `Capture / Export / Import / Apply`가 정상 동작
- 결석 플레이어용 extra session이 실제로 연결되는지 확인
...
- EXE만 설치해서 바로 실행 가능
- NPC 자동 대화, 소셜 연기, 자동 전투 지원
- Scene preset과 multi-session ownership 지원
...
- Scene preset import/export/apply
- multi-session ownership routing
- 전투 자동 턴 처리와 전술 판단 유지
...
1. Quick Setup
2. 소셜 연기 탭
3. `@NPC` world activity 입력
4. preset 적용 또는 Import
5. Discord 반응
6. 전투 자동 턴
...
4. 설치 후 `Install Prerequisites -> Codex Login -> Save Quick Setup -> Save Social -> Diagnostics -> Start` 순서로 실행한다
